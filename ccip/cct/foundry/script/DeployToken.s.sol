...
contract DeployToken is Script, Config {
    ...
    function deployToChain(uint256 chainId) internal {
        ...
        string memory name = HelperUtils.getStringFromJson(
            vm,
            configPath,
            ".BnMToken.name"
        );
        string memory symbol = HelperUtils.getStringFromJson(
            vm,
            configPath,
            ".BnMToken.symbol"
        );
        uint8 decimals = uint8(
            HelperUtils.getUintFromJson(vm, configPath, ".BnMToken.decimals")
        );
        uint256 maxSupply = HelperUtils.getUintFromJson(
            vm,
            configPath,
            ".BnMToken.maxSupply"
        );
        uint256 preMint = HelperUtils.getUintFromJson(
            vm,
            configPath,
            ".BnMToken.preMint"
        );
        ...
        BurnMintERC20 token = new BurnMintERC20(
            name,
            symbol,
            decimals,
            maxSupply,
        ...
        string memory key = string(
            abi.encodePacked("deployedToken_", chainName)
        );
