...
describe("Sender and Receiver", function () {
  ...
  /*
  it("gas CCIPReceive", async function () {
    const { sender, router, receiver } = await loadFixture(deployFixture);

    const abiCoder = new ethers.AbiCoder();

    const message = {
      messageId:
        "0x1f5d828a21976d4e71041581a862807c1ed2e1119562f7fe7e29e0092ebe946f",
      sourceChainSelector: chainSelector,
      sender: abiCoder.encode(["address"], [await sender.getAddress()]),
      data: abiCoder.encode(["uint256"], [1000]), // iterations
      destTokenAmounts: [], // no tokens
    };

    // compare

    await router.routeMessage(message, 5000, 600000, receiver);

    const mockRouterEvents = await router.queryFilter(
      router.filters.MessageExecuted
    );
    const mockRouterEvent = mockRouterEvents[mockRouterEvents.length - 1]; // check last event
    const gasUsed = mockRouterEvent.args.gasUsed;

    console.log("gasUSed comparison", gasUsed.toString());
  });
  */
});
